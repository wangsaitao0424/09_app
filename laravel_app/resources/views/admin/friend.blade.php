...
@section('content')
    ...
    <center class="smart-green">
        <label><span>友情链接名称 :</span>
            <input id="title" type="text"  name="f_name" placeholder="友情链接名称"/>
            <div class="error-msg"></div>
        </label>
        <label><span>友情链接地址 :</span>
            <input id="title" type="text"  name="f_url" placeholder="地址" /> &nbsp; &nbsp; &nbsp; &nbsp;
            <div class="error-msg"></div>
        </label>
        <label><span>导航栏权重 :</span>
            <input id="title" type="text"  name="f_weight" placeholder="请填数字" /> &nbsp; &nbsp; &nbsp; &nbsp;
            <div class="error-msg"></div>
        </label>
    ...
    <script>
        $(document).ready(function () {
            $(".but").on('click',function () {
                ...
                if(!f_name){
                    alert("友情链接名称不能为空");return;
                }
                let f_url=$("input[name='f_url']").val();
                let Expression=/http(s)?:\/\/([\w-]+\.)+[\w-]+(\/[\w- .\/?%&=]*)?/;
                let objExp=new RegExp(Expression);
                if(!f_url){
                    alert("导航栏地址不能为空");return;
                }else if(objExp.test(f_url) != true){
                    alert("网址格式不正确！请重新输入");return;
                }
                let f_weight=$("input[name='f_weight']").val();
                if(!f_weight){
                    alert("权重不能为空");return;
                }else if(isNaN(f_weight)){
                    alert("权重不全是数字");return;
                }
                ...
                $.ajax({
                    ...
                    success:function (msg) {
                        if(msg == 1){
                            alert("添加成功");
                            window.location.href="{{url('admin/friend_list')}}"
                        }else{
                            alert("添加失败");
                        }
                    }
