        ...
        <tbody>
            @foreach ($list_three as $v)
                <tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';" align="content">
                    ...
                    <td>{{$v['nth_content']}}</td>
                    <td><img src="{{$v['nth_img']}}" alt="" style="height: 50px;width: 50px"></td>
                    <td>{{date("Y-m-d H:i:s",$v['nth_time'])}}</td>
                    <td><a href="{{url('admin/navigation_bar_three_del?nth_id='.$v['nth_id'])}}" class="layui-btn layui-btn-normal">删除</a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
