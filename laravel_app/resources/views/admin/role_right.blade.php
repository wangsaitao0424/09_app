...
@section('content')
    ...
    <center class="smart-green">
        ...
        <label><span>权限地址 :</span>
            <select name="right_id" id="name2">
                <option value="">请选择</option>
                @foreach($right as $v)
                    <option value="{{$v['right_id']}}">{{$v['right_id']}}.{{$v['right_url']}}</option>
                @endforeach
            </select>
            <div class="error-msg"></div>
        </label>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(".but").on('click',function () {
                ...
                //     alert("请添加图片");return;
                // }
                let role_id=$('#name1 option:selected').val();
                // alert(user_id);
                if(!role_id){
                    alert("请选择");return;
                }
                let right_id=$('#name2 option:selected').val();
                if(!right_id){
                    alert("请选择");return;
                }
                // alert(role_name);return;
                ...
                let url="{{url('admin/role_right_do')}}";
                $.ajax({
                    data:data,
                    dataType: "json",
                    type: "post",
