...
@section('content')
    ...
    <center class="smart-green">
        <label><span>上轮播图片 :</span>
            <input type="file" id="uploadify" />
            <div id="show_img"></div>
            <div class="error-msg"></div>
        </label>
        <label><span>上轮播权重 :</span>
            <input id="title" type="text"  name="sl_weight" placeholder="请填数字" /> &nbsp; &nbsp; &nbsp; &nbsp;
            <div class="error-msg"></div>
        </label>
        ...
        $(document).ready(function () {
            $("#uploadify").uploadify({
                ...
                'onUploadSuccess':function (file,msg,data) {
                    let img="<img src='http://www.laravel_app.com/"+msg+"' alt='waht' style='width: 200px;height: 200px'> <input type='hidden' name='sl_img' value='"+msg+"'>";
                    $("#show_img").append(img);
                }
            });
            $(".but").on('click',function () {
                ...
                let sl_img=$("input[name='sl_img']").val();
                if(!sl_img){
                    alert("请添加图片");return;
                }
                let sl_weight=$("input[name='sl_weight']").val();
                if(!sl_weight){
                    alert("导航栏权重不能为空");return;
                }else if(isNaN(sl_weight)){
                ...
                $.ajax({
                    ...
                    success:function (msg) {
                        if(msg == 1){
                            alert("添加成功");
                            window.location.href="{{url('admin/slides_list')}}"
                        }else{
                            alert("添加失败");
                        }
                    }
