...
@section('content')
    ...
    <center class="smart-green">
        ...
        <label><span>用户名称 :</span>
            <select name="user_id" id="name1">
                <option value="">请选择</option>
                @foreach($user as $v)
                <option value="{{$v['u_id']}}">{{$v['u_email']}}</option>
                 @endforeach
            </select>
            <div class="error-msg"></div>
        </label>
        <label><span>角色名称 :</span>
            <select name="role_id" id="name2">
                <option value="">请选择</option>
                @foreach($role as $v)
    ...
    <script>
        $(document).ready(function () {
            ...
            $(".but").on('click',function () {
                ...
                //     alert("请添加图片");return;
                // }
                let user_id=$('#name1 option:selected').val();
                // alert(user_id);
                if(!user_id){
                    alert("请选择");return;
                }
                let role_id=$('#name2 option:selected').val();
                if(!role_id){
                    alert("请选择");return;
                }
                ...
                let url="{{url('admin/user_role_do')}}";
                $.ajax({
                    data:data,
                    dataType: "json",
                    type: "post",
